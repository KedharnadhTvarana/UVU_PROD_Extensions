...
{{!-- <a class="header-mini-cart-menu-cart-link {{#if showLines}}header-mini-cart-menu-cart-link-enabled{{/if}}"
	data-type="mini-cart" title="{{translate 'Cart'}}" data-touchpoint="{{cartTouchPoint}}" data-hashtag="#cart"
	href="#">
	<i class="header-mini-cart-menu-cart-icon">
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400" class="svg-shopping-trolley">
...
<div class="header-mini-cart">
	...
	<div class="header-mini-cart-buttons" data-view="MiniCart.Actions">
		<div class="header-mini-cart-buttons-left">
			<a href="#" class="header-mini-cart-button-view-cart" data-touchpoint="{{cartTouchPoint}}"
				data-hashtag="#cart">
				{{translate 'View Cart'}}
			</a>
		</div>
...
<script>
	$(document).on('focus', '.header-mini-cart-button-checkout', function () {
		$(this).on('keydown', function (e) {
			if (e.key === "Tab" && !e.shiftKey) {
				$('[data-view="Header.MiniCart"]').removeClass('open');
				e.preventDefault(); // Prevent default tab behavior
				e.stopPropagation();
				$('.header-mini-cart-menu-cart-link').focus(); // Set focus to header close button

			}
		});
	})
	$(document).on('focus', '.imageAttribute', function () {
		$(this).on('keydown', function (e) {
			if (e.key === "Tab" && e.shiftKey) {
				$('[data-view="Header.MiniCart"]').removeClass('open');
				e.preventDefault(); // Prevent default tab behavior
	...
	$(document).on('keydown', function (event) {
		if (event.key === 'Escape') {
			const focusableElements = [
				'.header-mini-cart-item-cell a',                // Links inside the mini-cart item cell
				'.header-mini-cart-item-cell-details ul li',   // List items in the details section
				'.header-mini-cart-buttons a',                // Buttons (View Cart, Checkout)
				'.header-mini-cart-item-cell-product-title a'  // Product title link
			];

			if ($(document.activeElement).is(focusableElements.join(','))) {
				$('[data-view="Header.MiniCart"]').removeClass('open');

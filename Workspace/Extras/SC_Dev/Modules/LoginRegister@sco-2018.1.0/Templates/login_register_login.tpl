...
<form class="login-register-login-form" novalidate>
	<fieldset class="login-register-login-form-fieldset">
		{{#if isSkipLogin}}
		<legend class="login-register-login-description" style="padding-top: 20px;">
			{{translate 'Login below or <a class="login-register-login-register-now" href="register"
				data-toggle="show-in-modal" data-type="register-now">create an account</a>'}}</legend>
		{{else}}
		<legend class="login-register-login-description" style="padding-top: 20px;">
			{{#if extraLoginRegisterLoginView.loginText}}{{{extraLoginRegisterLoginView.loginText}}}{{/if}}</legend>
		{{/if}}
		<small class="login-register-login-required">{{translate 'Required <span
				class="login-register-login-form-required">*</span>'}}</small>
		<div class="login-register-login-form-controls-group" data-validation="control-group">
			...
			<div class="login-register-login-form-controls" data-validation="control">
				<input {{#if hasAutoFocus}} autofocus {{/if}} type="email" name="email" id="login-email"
					class="login-register-login-form-input" placeholder="{{translate 'your@email.com'}}" required />
			</div>
		</div>

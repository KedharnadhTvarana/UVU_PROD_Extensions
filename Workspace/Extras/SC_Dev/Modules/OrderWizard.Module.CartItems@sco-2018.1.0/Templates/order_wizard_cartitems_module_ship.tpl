<section class="order-wizard-cartitems-module-ship">
	<div class="order-wizard-cartitems-module-ship-accordion-head">
		<button
			class="order-wizard-cartitems-module-ship-accordion-head-toggle{{#unless showOpenedAccordion}} collapsed{{/unless}}{{#if isAccordionPrimary}} order-wizard-cartitems-module-ship-accordion-primary{{/if}}"
			data-toggle="collapse" data-target="#unfulfilled-items-ship" aria-controls="unfulfilled-items">
			{{#if showAddress}}
			<span>{{translate 'Ship to '}}</span>
			<span
				class="order-wizard-cartitems-module-ship-accordion-head-toggle-address-name">&nbsp;{{address.fullname}}</span>
			<span> ({{itemCount}})</span>
			{{else}}
	...
	<div class="order-wizard-cartitems-module-ship-accordion-body collapse {{#if showOpenedAccordion}}in{{/if}}"
		...
		<div class="order-wizard-cartitems-module-ship-accordion-container" data-content="order-items-body">
			...
			<div class="order-wizard-cartitems-module-ship-dropdown">
				<a id="order-wizard-cartitems-module-ship-address-dropdown-{{address.id}}"
					class="order-wizard-cartitems-module-ship-address-dropdown-link" data-toggle="dropdown"
					aria-expanded="true">

					<span class="order-wizard-cartitems-module-ship-address-label">{{translate 'Ship to'}} </span>
					{{address.fullname}}
			...
			<div class="order-wizard-cartitems-module-ship-products-scroll">
				<div class="{{#if showMobile}}lg2sm-first{{/if}} order-wizard-cartitems-module-ship-table">
